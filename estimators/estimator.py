...
from data_loaders.hdf5_loader import DataIterator
...
class Estimator(object):
    ...
    def __init__(self, model, conf):
        ...
        self.summary_manager = SummaryManager(conf.summary_dir, tf.compat.v1.get_default_graph(), conf.freq_list)
        ...
        try:
            self.sess.run(tf.compat.v1.assert_variables_initialized())
...
class SummaryManager(object):
    def __init__(self, summary_dir, model_graph, freq_list, mode='train'):
        ...
        self.trainWriter = tf.compat.v1.summary.FileWriter(self.train_dir,model_graph)
        self.validWriter = tf.compat.v1.summary.FileWriter(self.valid_dir,model_graph)
...
class NumbersKeeper(object):
    ...
    def AssignNpToTfVariables(self,count_dict):
        ...
        for mode,val1 in count_dict.items():
            if isinstance(val1,collections.Mapping):
                for freq, val in count_dict[mode].items():
                    assign_ops.append(tf.compat.v1.assign(self.tf_counts[mode][freq],val))
            else:
                assign_ops.append(tf.compat.v1.assign(self.tf_counts[mode],val1))
