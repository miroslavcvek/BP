...
class FCMultiScaleResidualDenseNet(object):
    def __init__(self,
        ...
        self.initializer = tf.keras.initializers.VarianceScaling(scale=2.0,
                                                                 mode='fan_in', seed=None)
        self.regularizer = tf.keras.regularizers.L2(l2=1.0)
        self.is_training = tf.compat.v1.placeholder(tf.bool)
        self.num_channels = inputs.get_shape()[-1]
    ...
    def NetworkForwardPass(self, n_feat_first_layer, n_pool, growth_rate, n_layers_per_block, dropout_rate):
        ...
        tf.compat.v1.summary.image('inputs', inputs, max_outputs=4, collections=['per_100_steps'])
        tf.compat.v1.summary.image('ground_truth', targets[:, :, :, None], max_outputs=4, collections=['per_100_steps'])

        stack = []
        # Inception Block
        l = tf.compat.v1.layers.Conv2D(
        ...
        #       Outputs     #
        #####################
        self.logits = tf.compat.v1.layers.Conv2D(inputs=block_to_upsample,
        ...
        tf.compat.v1.summary.image('predictions', tf.cast(self.predictions[:, :, :, None], tf.float32), max_outputs=4,
                                   collections=['per_100_steps'])

    def Optimize(self):
        update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys)
        with tf.control_dependencies(update_ops):
            self.train_op = self.optimizer.minimize(self.loss_op, global_step=tf.compat.v1.train.get_global_step())
        ...
        for metric_name in self.metrics_list:
            ...
            if metric_name == 'sW_CE_loss':
                ...
                with tf.compat.v1.variable_scope(metric_name):
                    ...
                    tf.compat.v1.summary.scalar('per-step', metric_obj.op, collections=['per_step'])
                    tf.compat.v1.summary.scalar('epoch-avg', metric_obj.avg, collections=['per_epoch'])
                ...
                with tf.compat.v1.variable_scope(metric_name):
                    L2_loss = tf.contrib.layers.apply_regularization(
                        tf.keras.regularizers.L2(scale=self.weight_decay),
    ...
    def Summaries(self):
        self.summary_ops = {}
        self.summary_ops['1step'] = tf.compat.v1.summary.merge_all(key='per_step')
        self.summary_ops['100steps'] = tf.compat.v1.summary.merge_all(key='per_100_steps')
        self.summary_ops['1epoch'] = tf.compat.v1.summary.merge_all(key='per_epoch')
...
if __name__ == '__main__':
    ...
    num_channels = 1
    num_class = 4
    inputs = tf.compat.v1.placeholder(tf.float32, shape=(None, None, None, num_channels))
    targets = tf.compat.v1.placeholder(tf.uint8, shape=(None, None, None))
    weight_maps = tf.compat.v1.placeholder(tf.float32, shape=[None, None, None])
