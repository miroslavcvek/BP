...
def BN_ELU_Conv(inputs, n_filters, filter_size=3, dropout_rate=0.2, is_training=tf.constant(False,dtype=tf.bool)):
    initializer = tf.keras.initializers.VarianceScaling(factor=2.0, mode='FAN_IN', uniform=False, seed=None, dtype=tf.float32)
    regularizer = tf.keras.regularizers.L2(scale=1.0)

    l = tf.keras.layers.BatchNormalization(inputs, activation_fn=tf.nn.elu,
                        is_training=is_training)
    ...
    l = tf.keras.layers.Dropout(l, rate=(dropout_rate), training=is_training)
    return l

def TransitionDown(inputs, n_filters, dropout_rate=0.2, is_training=tf.constant(False,dtype=tf.bool)):
    l = BN_ELU_Conv(inputs, n_filters, filter_size=1, dropout_rate=dropout_rate,
                     is_training=is_training)
    l = tf.keras.layers.MaxPool2D(l, pool_size=[2, 2], strides=2)
    return l
...
def ResidualTransitionUp(skip_connection, block_to_upsample, n_filters_keep,
    ...
    Performs upsampling on block_to_upsample by a factor 2 and adds it with the skip_connection
    """
    initializer = tf.keras.initializers.VarianceScaling(factor=2.0,
                mode='FAN_IN', uniform=False, seed=None, dtype=tf.float32)
    regularizer = tf.keras.regularizers.L2(scale=1.0)
...
def Dice2Class(output, target, main_class, smooth = 1.0, name = 'dice_score'):
    ...
    targets_unrolled = tf.reshape(target,[-1,num_class])[:,main_class]
    intersection = tf.reduce_sum(preds_unrolled*targets_unrolled)
    ret_val = (2.0*intersection)/(tf.reduce_sum(preds_unrolled) + tf.reduce_sum(targets_unrolled) + smooth)
    ret_val = tf.identity(ret_val,name = 'dice_score')
    return ret_val
...
class MetricStream(object):
    def __init__(self,op):
        ...
        self.sum = tf.Variable([0.0,0], name = op.name[:-2] + '_sum', trainable = False)
        self.avg = tf.Variable(0.0, name = op.name[:-2] + '_avg', trainable = False)
        self.accumulate = tf.compat.v1.assign_add(self.sum,[self.op,count])
        self.reset = tf.compat.v1.assign(self.sum,[0.0,0.0])
        self.stats = tf.compat.v1.assign(self.avg,self.sum[0]/(0.001 + self.sum[1]))